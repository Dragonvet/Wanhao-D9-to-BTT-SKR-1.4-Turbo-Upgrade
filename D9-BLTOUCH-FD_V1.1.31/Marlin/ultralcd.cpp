...
#define MY_MENU_ITEM(TYPE, LABEL, offset,...) do { \
  ...
  void lcd_control_temperature_menu() {
    ...
    #endif 
    #if HAS_TEMP_BED
    #ifdef FYS_BLUEMODEL_CUSTOMIZED
        MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, MSG_BED, &thermalManager.target_temperature_bed, 0, BED_MAXTEMP, watch_temp_callback_bed);
    #else
        MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, MSG_BED, &thermalManager.target_temperature_bed, 0, BED_MAXTEMP - 15, watch_temp_callback_bed);
    #endif
    #endif
  ...
  void lcd_control_filament_menu() {
    ...
    void lcd_advanced_print_break_wait_for_nozzles_to_heat() {
      START_SCREEN();
      STATIC_ITEM("PRINT BREAK", true, true);
      STATIC_ITEM(MSG_FILAMENT_CHANGE_HEATING_1);
      #ifdef MSG_FILAMENT_CHANGE_HEATING_2
        STATIC_ITEM(MSG_FILAMENT_CHANGE_HEATING_2);
        #define _FC_LINES_C 3
    ...
    void lcd_advanced_pause_show_message(const AdvancedPauseMessage message) {
      switch (message) {
        ...
        case ADVANCED_PAUSE_MESSAGE_WAIT_FOR_NOZZLES_TO_HEAT:
          ...
          break;
        #ifdef FYS_PRINT_BREAK_HEATUP_FIRST
          case ADVANCED_PRINT_BREAK_MESSAGE_HEAT_NOZZLE:
          defer_return_to_status = true;
          lcd_goto_screen(lcd_advanced_print_break_wait_for_nozzles_to_heat);
          break;
        #endif
...
#if ENABLED(FYS_LCD_EVENT)&&ENABLED(FYS_SAFE_PRINT_BREAK)
static void FunV042()
{
  uint8_t n;
  for (n = 0; n < 16; n++)
...
#endif
void lcd_update() {
  #ifdef ENABLED(FYS_LCD_EVENT)&&ENABLED(FYS_SAFE_PRINT_BREAK)
    FunV042();
  #endif
  #if ENABLED(ULTIPANEL)
